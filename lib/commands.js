const StructsDB = `java` +
  ` -cp /usr/local/transitclock/Core.jar org.transitclock.applications.SchemaGenerator` +
  ` -p org.transitclock.db.structs` +
  ` -o /usr/local/transitclock/db`

const WebStructsDB = `java` +
  ` -cp /usr/local/transitclock/Core.jar org.transitclock.applications.SchemaGenerator` +
...
const StartCore = (propertiesFile, secondary = false) => `nohup` +
  ` java` +
  ` -Dtransitclock.configFiles=${propertiesFile}` +
  (secondary === true ? ` -Dtransitclock.rmi.secondaryRmiPort=0` : ``) +
  ` -Dtransitclock.logging.dir=/tmp` +
  ` -jar /usr/local/transitclock/Core.jar` +
  ` &`

const InitRMI = `rmiregistry &`

const PutAPIKeyInEnv = apikey => `export APIKEY=${apikey}`
const ExportAPIKey = apikey => `export CATALINA_OPTS="$CATALINA_OPTS -Dtransitclock.apikey=${apikey}" && export JAVA_OPTS="$JAVA_OPTS -Dtransitclock.apikey=${apikey}"`
const ExportConfigFiles = configs => `export JAVA_OPTS="$JAVA_OPTS -Dtransitclock.configFiles=${configs}"`

const StartTomcat = `/usr/local/tomcat/bin/startup.sh`
